...
#
# Writes: dist/Destruction-Derby-2-Recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix Destruction-Derby-2-Recomp \
  --exe-name Destruction_Derby_2_Recompiled \
  --display-name "Destruction Derby 2 Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Destruction Derby 2 disc" \
