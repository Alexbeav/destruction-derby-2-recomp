# Destruction Derby 2 release feasibility
...
The supported serial set is `SCUS-94350`. The package uses
...
## v0.3.4 executable-name correction

Public `v0.3.0` can complete a build and then request the wrong executable.
The corrected source uses `Destruction_Derby_2_Recompiled` for CMake, the setup relaunch, and packaging.
...
The first corrected package passed the canonical Track 01, merged-CUE,
generation, rebuild, automatic-launch, and bounded runtime gates on Pegasus.
